...
// Package types contains data types related to Kaleido consensus.
package types
...
type Header struct {
	...
	MixDigest   common.Hash    `json:"mixHash"` // used as seed in kaleido
	Nonce       BlockNonce     `json:"nonce"`   // used as consensus version in kaleido
...
func (h *Header) SetSeedProof(proof []byte) {
	copy(h.Certificate.Proposal.SeedProof[:], proof[:])
}

func (h *Header) SeedProof() ed25519.VrfProof {
	return h.Certificate.Proposal.SeedProof
}

func (h *Header) SeedProofBytes() []byte {
	return h.Certificate.Proposal.SeedProof[:]
}
...
func (h *Header) SetProposer(addr common.Address) {
	h.Certificate.Proposal.Credential.Address = addr
}
...
func (b *Block) SeedProof() ed25519.VrfProof    { return b.header.Certificate.Proposal.SeedProof }
