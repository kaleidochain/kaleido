...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Kaleido protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of KALs to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Kaleido and the faucet")
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s KALs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	if *genesisFlag == "" {
		genesis = core.DefaultKaleidoTestnetGenesisBlock()
	} else {
...
// apiHandler handles requests for KAL grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/kaleidochain/kaleido/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No address found to fund")
	}
	var avatar string
