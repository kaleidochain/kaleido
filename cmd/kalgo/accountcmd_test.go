...
func TestAccountListEmpty(t *testing.T) {
	kal := runKalgo(t, "account", "list")
	kal.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	kal := runKalgo(t, "account", "list", "--datadir", datadir)
	defer kal.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	kal := runKalgo(t, "account", "new", "--lightkdf")
	defer kal.ExpectExit()
	kal.Expect(`
...
`)
	kal.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	kal := runKalgo(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer kal.ExpectExit()
	kal.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(kal.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(kal.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	kal.SetTemplateFunc("keypath", func(file string) string {
