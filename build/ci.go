...
import (
	...
	"github.com/kaleidochain/kaleido/internal/build"
	"github.com/kaleidochain/kaleido/params"
)

var (
	// Files that end up in the kalgo*.zip archive.
	kalgoArchiveFiles = []string{
		"COPYING",
		executablePath("kalgo"),
	}

	// Files that end up in the kalgo-alltools*.zip archive.
	allToolsArchiveFiles = []string{
		"COPYING",
		executablePath("kalabigen"),
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 9 {
			...
			log.Println("kaleido requires at least Go version 1.9 and cannot")
...
func doArchive(cmdline []string) {
	...
	if err := build.WriteArchive(geth, kalgoArchiveFiles); err != nil {
		log.Fatal(err)
	}
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "kalgo-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "io.kaleidochain", "-v", "github.com/kaleidochain/kaleido/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("kalgo.aar", filepath.Join(GOBIN, "kalgo.aar"))
		return
	}
...
func doAndroidGengineArchive(cmdline []string) {
	...
	build.MustRun(goTool("get", "golang.org/x/mobile/cmd/gomobile", "golang.org/x/mobile/cmd/gobind"))
	build.MustRun(gomobileTool("init", "--ndk", os.Getenv("ANDROID_NDK")))
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "io.kaleidochain", "-v", "github.com/kaleidochain/kaleido/mobile_gengine"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "--tags", "ios", "-v", "github.com/kaleidochain/kaleido/mobile")
...
func doXCodeGengineFramework(cmdline []string) {
	...
	build.MustRun(goTool("get", "golang.org/x/mobile/cmd/gomobile", "golang.org/x/mobile/cmd/gobind"))
	build.MustRun(gomobileTool("init"))
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "--tags", "ios", "-v", "github.com/kaleidochain/kaleido/mobile_gengine")
